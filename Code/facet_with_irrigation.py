...
#stem_df = pd.read_csv("Clean_Data/SWP_averaged_Long.csv", parse_dates={'date_time': [0]}, dayfirst = True) 
...
# Stack dataframes
frames = [soil_df, irr_df]
df = pd.concat(frames)


# Create facet plot using seaborn
#sns.set_style("whitegrid")
g = sns.FacetGrid(df, col="Area", hue="Sensor", col_wrap=3, palette="Set1",)
g.map(plt.plot, "date_time", "value", linewidth=0.5) 
g.add_legend() 
g.set_axis_labels("Time", "VWC (%)/mm of water") 
...
plt.show()
plt.savefig("Output/facet_bad_data.pdf")
